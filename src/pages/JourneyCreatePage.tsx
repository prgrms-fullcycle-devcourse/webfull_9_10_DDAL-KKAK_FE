...
import type { CurrencyCode, RateMode } from '../types'
...
export function JourneyCreatePage() {
  ...
  const authName =
    auth.status === 'logged_in' ? auth.user.name.trim() : '나'

  const today = useMemo(() => new Date().toISOString().slice(0, 10), [])

  /** 신규 생성만 2단계 (여행지·이름 → 날짜·동행자) */
  ...
  useEffect(() => {
    ...
    setDates({
      start: existingJourney.startDate,
      end: existingJourney.endDate,
    })
    setParticipants(
      existingJourney.participants.length ? existingJourney.participants : ['나'],
    )
    setHydratedFromId(existingJourney.id)
  }, [isEdit, existingJourney, hydratedFromId])

  useEffect(() => {
    // 로그인한 유저는 자동으로 participants[0] + selfParticipant가 되도록 맞춤
    setParticipants((prev) => {
      const trimmed = authName || '나'
      const list = prev.length ? prev : ['나']
      if (list.includes(trimmed)) {
        // 항상 첫 번째로 올려서 ledgerSelfName fallback에도 안전하게
  ...
  const effectiveRate = (): number => {
    if (rateMode === 'fixed') {
      return typeof fixedRate === 'number' ? fixedRate : 0
    }
    return demoRateForCurrency(currency)
  }

  const saving = createJourney.isPending || updateJourneyMut.isPending

  const createDefaultsFromForm = () => {
    const seed = destination.trim() || name.trim()
    const r = infer(seed)
    const cur = r?.currency ?? 'JPY'
    const cou = r?.country ?? '기타'
    const rate = r?.fixedRate ?? demoRateForCurrency(cur)
    return { cur, cou, rate }
  }
  ...
  const createStep1 = (
    <>
      <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3">
        <p className="text-xs font-black text-blue-900">Step 1 / 2</p>
        <p className="mt-1 text-sm font-bold leading-relaxed text-blue-800/90">
          돈 걱정은 나중에 하세요. 일단 여행부터 만들까요?
        </p>
      </div>
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-2 px-1 text-[10px] font-bold text-slate-400">
            자동 인식: <span className="text-slate-700">{country}</span>
          </p>
        </div>
        <div>
          <label className="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">
  ...
  const createStep2 = (
    <>
      <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
        <p className="text-xs font-black text-slate-700">Step 2 / 2</p>
        <p className="mt-1 text-sm font-bold text-slate-500">
          날짜와 동행자만 정하면 바로 대시보드로 갈게요.
        </p>
      </div>
    ...
    <div className="min-h-dvh bg-white">
      <TopBar
        title={
          isEdit ? (
            <span>
              여정 수정 <Plane className="ml-1 inline size-5 text-blue-500" />
            </span>
          ) : (
            <span>
              여정의 시작 <Plane className="ml-1 inline size-5 text-blue-500" />
            </span>
          )
        }
        onBack={() => {
          if (!isEdit && createStep === 2) setCreateStep(1)
          else if (isEdit && journeyId) nav(`/journeys/${journeyId}`)
          else nav('/')
        }}
      />

      <main className="space-y-8 px-6 pb-28">
        {isEdit ? fullEditForm : createStep === 1 ? createStep1 : createStep2}
      </main>

      <div className="fixed inset-x-0 bottom-0 mx-auto w-full max-w-md bg-white p-6">
        {!isEdit && createStep === 1 ? (
          <button
            type="button"
            onClick={() => setCreateStep(2)}
            className="w-full rounded-3xl bg-blue-600 py-5 text-xl font-black tracking-tight text-white shadow-xl shadow-blue-100 active:scale-[0.99]"
          >
            다음
          </button>
        ) : (
          <button
            type="button"
            disabled={saving || (isEdit && !journeyId)}
            onClick={async () => {
              const safeName =
                name.trim() || destination.trim() || country || '새 여행'
              ...
              const { cur, cou, rate } = createDefaultsFromForm()
